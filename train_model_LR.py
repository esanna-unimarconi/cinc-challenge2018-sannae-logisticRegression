...
p_WINDOW_SIZE=60*Fs
p_INPUT_FEAT=1
p_OUTPUT_CLASS=1  # 1,0,-1 (total of 3)
p_BATCH_SIZE=10
p_EPOCHS=120
p_MODEL_NAME="LOG_REGR"
...
def LogisticRegressionKeras():
    ...
    model.add(Dense(OUTPUT_SIZE, activation='hard_sigmoid'))  #better perform relu or hard_sigmoid
...
def classify_record(record_name, model):
    ...
    for j in range(0, len(model_list)):
        ...
        try:
            model.load_weights('models/' + str(os.path.basename(this_model)))
            #model.load_weights('models/'+str(this_model) + str(p_MODEL_FILE))
        except Exception as e:
            print("non ho trovato i pesi (file " + str(p_MODEL_FILE) + "). parto a  zero")
            print(str(str(e)))
        ...
        predictions = model.predict(X_subj, batch_size=p_BATCH_SIZE)
        ...
        predictions = np.append(predictions, np.zeros(recordLength
    ...
    try:
        ...
        predictions=np.ravel(predictions)
    print("Predictions shape after: " + str(predictions.shape))
    
    predictions = [x * np.ones([window_size]) for x in predictions]
    predictions = np.concatenate(predictions)
    predictions = np.append(predictions, np.zeros(recordLength
                                                  - np.size(predictions, 0)))
    print("Predictions shape return: " + str(predictions.shape))
    """
    #return  None,predictions, model
    return None, predictions_mean, model